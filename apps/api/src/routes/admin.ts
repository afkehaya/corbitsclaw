...
import { getMarginPercent, setMarginPercent } from '../services/config.js';
...
adminRoutes.get('/users', async (c: Context) => {
  ...
  const usersWithBalances = await Promise.all(
    users.map(
      async (user: { id: string; email: string; created_at: string }) => {
...
adminRoutes.get('/users/:userId', async (c: Context) => {
  ...
  const { data: user, error: userError } = await supabase
    ...
    .select('id, email, created_at')
    .eq('id', userId)
    .single<{ id: string; email: string; created_at: string }>();
...
adminRoutes.get('/metrics', async (c: Context) => {
  ...
  const totalRevenue = (deposits as DepositRow[]).reduce(
  ...
  if (totalsError) {
    throw new Error(
      `Failed to fetch transaction totals: ${totalsError.message}`
    );
  }
  ...
  const totalApiCost = (transactionTotals as TotalsRow[]).reduce(
    (sum: number, t) => sum + Number(t.cost_x402),
    0
  );

  const totalMarginEarned = (transactionTotals as TotalsRow[]).reduce(
    (sum: number, t) => sum + Number(t.cost_margin),
    0
  );
...
adminRoutes.put('/settings', async (c: Context) => {
  ...
  if (
    ...
    throw new InvalidRequestError(
      'marginPercent is required and must be a number'
    );
  }
  ...
  const result = await setMarginPercent(body.marginPercent);
