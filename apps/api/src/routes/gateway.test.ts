 ...
 */
function createTestApp(state: TestState, options: {
  ...
  const mockAuthMiddleware = async (c: Context, next: () => Promise<void>) => {
    ...
    const user = Object.values(TEST_USERS).find((u) => u.api_key === token);
    if (!user) {
      throw new AuthError("Invalid API key");
    }
  ...
  const mockHasSufficientBalance = async (userId: string, amount: number): Promise<boolean> => {
  ...
  const mockMakeX402Request = async (_baseUrl: string, _path: string, _body?: unknown) => {
    ...
    const costPaid = "x402CostPaid" in options ? options.x402CostPaid : "1000000";

    return {
      data: options.x402Response ?? { success: true, model: "test-model" },
      response: new Response(JSON.stringify(options.x402Response ?? {}), { status: 200 }),
      costPaid,
    };
  };
  ...
  const handleGatewayRequest = async (
    ...
    // Check balance
    const hasBalance = await mockHasSufficientBalance(user.id, MIN_BALANCE_THRESHOLD);
    if (!hasBalance) {
    ...
    try {
      const result = await mockMakeX402Request(`https://${endpoint}.corbits.dev`, path, body);
      data = result.data;
      costX402 = result.costPaid ? Number(BigInt(result.costPaid)) / Math.pow(10, USDC_DECIMALS) : 0;
    } catch (error) {
...
test("Authentication: request without API key returns 401", async () => {
  ...
  const response = await makeRequest(app, "POST", "/gateway/xai/v1/chat/completions", {
...
test("Authentication: request with valid API key proceeds", async () => {
  ...
  const response = await makeRequest(app, "POST", "/gateway/xai/v1/chat/completions", {
    headers: { Authorization: `Bearer ${TEST_USERS.validUser.api_key}` },
    body: { model: "grok-beta", messages: [{ role: "user", content: "Hello" }] },
  });
...
test("Balance check: user with zero balance gets 402", async () => {
  ...
  const response = await makeRequest(app, "POST", "/gateway/xai/v1/chat/completions", {
    headers: { Authorization: `Bearer ${TEST_USERS.zeroBalanceUser.api_key}` },
    body: { model: "grok-beta", messages: [{ role: "user", content: "Hello" }] },
  });
...
test("Balance check: user with sufficient balance proceeds", async () => {
  ...
  const response = await makeRequest(app, "POST", "/gateway/xai/v1/chat/completions", {
    headers: { Authorization: `Bearer ${TEST_USERS.richUser.api_key}` },
    body: { model: "grok-beta", messages: [{ role: "user", content: "Hello" }] },
  });
...
test("Proxy flow: valid request to OpenAI endpoint returns response", async () => {
  ...
  const response = await makeRequest(app, "POST", "/gateway/openai/v1/chat/completions", {
    headers: { Authorization: `Bearer ${TEST_USERS.validUser.api_key}` },
...
test("Proxy flow: valid request to Amazon endpoint returns response", async () => {
  const state = createTestState();
  const mockResponse = {
    output: { message: { role: "assistant", content: [{ text: "Greetings!" }] } },
  };
  ...
  const response = await makeRequest(app, "POST", "/gateway/amazon/model/invoke", {
    headers: { Authorization: `Bearer ${TEST_USERS.validUser.api_key}` },
...
test("Error handling: Corbits endpoint error returns 502", async () => {
  ...
  assert.ok(body.message.includes("timeout") || body.message.includes("upstream"));
...
test("Cost tracking: X-Request-Id header is returned", async () => {
  ...
  assert.ok(requestId.startsWith("req_"), "Request ID should have correct prefix");
});
...
test("Multiple endpoints: all three endpoints work correctly", async () => {
  ...
  // Test xAI
  const xaiResponse = await makeRequest(app, "POST", "/gateway/xai/v1/chat/completions", {
    headers: { Authorization: `Bearer ${TEST_USERS.richUser.api_key}` },
    body: { model: "grok-beta", messages: [] },
  });
  ...
  // Test OpenAI
  const openaiResponse = await makeRequest(app, "POST", "/gateway/openai/v1/chat/completions", {
    headers: { Authorization: `Bearer ${TEST_USERS.richUser.api_key}` },
    body: { model: "gpt-4", messages: [] },
  });
  ...
  // Test Amazon
  const amazonResponse = await makeRequest(app, "POST", "/gateway/amazon/model/invoke", {
    headers: { Authorization: `Bearer ${TEST_USERS.richUser.api_key}` },
...
test("Multiple endpoints: paths are extracted correctly", async () => {
  ...
  await makeRequest(app, "POST", "/gateway/amazon/bedrock/model/invoke-stream", {
    headers: { Authorization: `Bearer ${TEST_USERS.richUser.api_key}` },
    body: {},
  });
  ...
  assert.strictEqual(state.transactions[2]!.path, "/bedrock/model/invoke-stream");
});
...
test("Edge case: request without body succeeds", async () => {
  ...
  const request = new Request("http://localhost/gateway/xai/v1/models", {
    method: "POST",
    headers: {
      Authorization: `Bearer ${TEST_USERS.validUser.api_key}`,
    },
  });
...
test("Edge case: multiple requests deplete balance correctly", async () => {
  ...
  const response1 = await makeRequest(app, "POST", "/gateway/xai/v1/chat/completions", {
    headers: { Authorization: `Bearer ${TEST_USERS.validUser.api_key}` },
    body: { model: "grok-beta", messages: [] },
  });
  ...
  const response2 = await makeRequest(app, "POST", "/gateway/xai/v1/chat/completions", {
    headers: { Authorization: `Bearer ${TEST_USERS.validUser.api_key}` },
    body: { model: "grok-beta", messages: [] },
  });
  ...
  const response3 = await makeRequest(app, "POST", "/gateway/xai/v1/chat/completions", {
    headers: { Authorization: `Bearer ${TEST_USERS.validUser.api_key}` },
    body: { model: "grok-beta", messages: [] },
  });
