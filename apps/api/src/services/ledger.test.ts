...
import type { CreditEntry, Transaction } from "@openclawd/shared";
...
test("getBalance", async (t: Test) => {
  ...
  await t.test("returns correct sum of deposits and usage", async (t: Test) => {
    ...
    t.equal(balance, 125, "should return sum of all amounts (100 - 25 + 50 = 125)");
  });
  ...
  await t.test("handles negative amounts correctly", async (t: Test) => {
    ...
    t.equal(balance, -25, "should correctly handle negative balance (50 - 75 = -25)");
  });
...
test("recordDeposit", async (t: Test) => {
  ...
  await t.test("stores stripe session ID", async (t: Test) => {
    ...
    t.equal(result.stripeSessionId, "sess_xyz", "should store stripe session ID");
  });
  ...
  await t.test("returns correct CreditEntry structure", async (t: Test) => {
    ...
    t.equal(result.description, "Credit deposit via Stripe", "should have default description");
...
test("recordUsage", async (t: Test) => {
  ...
  await t.test("returns correct CreditEntry structure", async (t: Test) => {
    ...
    const result = await recordUsage("user-123", 20, "req_test", "Claude API call");

    t.ok(result.id, "should have id");
    t.equal(result.userId, "user-123", "should have correct userId");
    t.equal(result.amount, -20, "should have negative amount");
    t.equal(result.type, "usage", "should have type usage");
    t.equal(result.description, "Claude API call", "should have custom description");
    t.equal(result.requestId, "req_test", "should have requestId");
    t.ok(result.createdAt instanceof Date, "should have createdAt as Date");
  });

  await t.test("uses default description when not provided", async (t: Test) => {
    resetMocks();

    const result = await recordUsage("user-123", 15, "req_default");

    t.equal(result.description, "API usage", "should use default description");
  });
...
test("getUsageHistory", async (t: Test) => {
  ...
  await t.test("calculates total correctly", async (t: Test) => {
    ...
    t.equal(result.total, 0.35, "total should be sum of costTotal (0.10 + 0.25 = 0.35)");
  });
  ...
  await t.test("uses default 30 days when days not specified", async (t: Test) => {
    resetMocks();
    mockTransactionsData = [];
...
test("recordTransaction", async (t: Test) => {
  ...
  await t.test("handles optional fields", async (t: Test) => {
    ...
    t.ok(result.id, "should have id");
    t.equal(result.responseStatus, undefined, "responseStatus should be undefined");
    t.equal(result.responseTimeMs, undefined, "responseTimeMs should be undefined");
  });
});
