...
import { getMarginPercentSync } from './config.js';
...
export async function createCheckoutSession(
  ...
  const priceWithMargin = Math.round(
    amountUsd * (1 + marginPercent / 100) * 100
  ); // in cents
...
async function processCompletedCheckout(
  ...
  if (lookupError && lookupError.code !== 'PGRST116') {
    ...
    throw new Error(
      `Failed to check for existing deposit: ${lookupError.message}`
    );
  }
  ...
  console.log(
    `Processed deposit: ${creditAmount} credits for user ${userId}, session ${session.id}`
  );
